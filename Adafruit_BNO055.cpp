...
/*!
 *  @brief  Sen interrupt action
 */
void Adafruit_BNO055::setAnyMotionInterrupt(uint8_t threshold, uint8_t duration, uint8_t axis, uint8_t int_type, uint8_t int_mask) {
  adafruit_bno055_opmode_t modeback = _mode;
  ...
  write8(ACC_AM_THRES,threshold);
  write8(INT_EN,int_type);
  write8(ACC_INT_SETTINGS, (duration | axis) );
  write8(INT_MSK, int_mask );
  /* Set the requested operating mode (see section 3.3) */
  setMode(modeback);
  delay(20);
}
...
void Adafruit_BNO055::resetInterrupts() {
  ...
  delay(25);
  write8(BNO055_PAGE_ID_ADDR, 0);
  write8(BNO055_SYS_TRIGGER_ADDR, 0X40);
  /* Set the requested operating mode (see section 3.3) */
  setMode(modeback);
  delay(20);
}
